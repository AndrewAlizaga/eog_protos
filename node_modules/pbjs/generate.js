...
function generate(schema, options) {
    ...
    for (const def of schema.messages) {
        if (typescript) {
            ...
            for (const field of def.fields) {
                ...
                if (field.map !== null) {
                    ...
                    if (from === 'bool' || from === 'string')
                        from = 'string';
                    else if (protoToType[from] === 'number')
                        from = 'number';
                    else if (protoToType[from] === 'Long')
                        from = 'string';
                    else
                        throw new Error(`The type ${from} cannot be used as a map key`);
        ...
        for (const field of def.fields) {
            ...
            if (field.map !== null)
                humanType += `<${field.map.from}, ${field.map.to}>`;
        ...
        for (const field of def.fields) {
            ...
            if (field.map !== null) {
                ...
                lines.push(`            case 2: {`);
                if (value.before)
                    lines.push(`              ${value.before};`);
                lines.push(`              value = ${value.read};`);
                if (value.after)
                    lines.push(`              ${value.after};`);
                lines.push(`              break;`);
                lines.push(`            }`);
                lines.push(`            default:`);
                ...
                if (from === 'bool')
                    lines.push(`        values[key + ''] = value;`);
                else if (protoToType[from] === 'Long')
                    lines.push(`        values[longToString(key)] = value;`);
                else
                    lines.push(`        values[key] = value;`);
